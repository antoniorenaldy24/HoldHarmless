...
**A-27 measured, and the bar it was given cannot be met (2026-09-26).** `pnpm a27` runs §6.6's twenty Block-C utterances — a cue phrase with the representative carrying straight on — through the real semantic layer, suspicion and gate, over perfect transcripts. The result is uniform and leaves nothing to interpret:

| | |
|---|---|
| Gate closed on the cue | **20 of 20.** The list is decisive, as §6.5 intends |
| Reopened | **20 of 20**, after **exactly two turns** every time — never more, never fewer |
| `gate_false_close_count` | **20 of 20**, derived from the log |
| Never reopened | 0 of 20. The latch §6.8 warns about does not occur once the assembly ends turns |

**The cost is therefore structural, not probabilistic: two far-end turns, always.** §6.5 clears suspicion at N=2, and a turn is as long as a person's sentence. Measured over the 28 rendered representative lines: minimum **1872 ms**, median 3696, p75 5112. So the least this can ever cost is **3744 ms**, and at the median **7392 ms** — against A-27's bar of **1500 ms**. No implementation of §6.5 as written can reach it, and no recognizer improvement helps, because the transcripts here are already perfect.

**The options, for the project owner.** Each changes a decision record, so none is taken here:

1. **Change the bar to a structural one.** "The gate reopens within two far-end turns, always" is what the design actually guarantees, and it is a stronger statement than a wall-clock p90 because it cannot drift with how talkative a representative is. The wall-clock figure is then reported, not bounded. *This is the measurement's own reading: the mechanism is deliberate (§6.5's "suspect fast, confirm slowly") and the bar is the part that was never measured against it.*
2. **Clear suspicion at N=1 outside `HOLD`.** Halves the cost to one turn. It also means a single mis-read `HUMAN` observation during a genuine hold reopens the gate and lets the agent speak over hold music — the exact failure A-7 and §6.7 exist to prevent.
3. **Clear on the same turn that carried the cue, when the rest of that turn reads `HUMAN`.** In all twenty trials the cue and the continuation are ONE utterance — "Let me check — okay, I see it right here" — and `holdCue`'s weight of 1.0 makes the whole of it `HOLD_CUE`. A rule that re-examined the utterance after the matched phrase would target exactly the measured case. It is a new mechanism, and it needs its own evidence.
4. **Do not close the gate on a cue alone, and wait for acoustic agreement.** Removes the cost entirely and gives up what §6.3 is for: the cue fires *before* hold audio starts and works on silent holds, which is the case the acoustic layer handles worst.
...
**What the path can and cannot measure.** It measures delay VARIATION: `mic_frame_delay_ms` is each frame's lateness against the fastest arrival-to-stream-position relationship seen so far, which is what a pipe stall or a GC pause looks like and what actually breaks live audio. It does **not** measure the acoustic path — microphone, driver, and the device's own buffering before ffmpeg sees anything — which needs a loopback reference and is not attempted. So `HUMAN_REP` capture latency is reported as a **lower bound**, and this section's rule that the two modes are never pooled is what keeps an unmeasured offset out of a `BOT_REP` figure. Every session reports `rep_mode`, and every `perceived_response_ms` figure carries its mode, so pooling them is not something prose has to ask an analyst not to do.

**Run against real hardware, which found a bug no test would have.** The one capture device on the demo host opens in **851 ms** and delivers a **flush**: 174 frames — 3480 ms of audio — inside the first 3000 ms of wall time, because ffmpeg buffers the device before writing. Two consequences, both now measured rather than assumed:

- **851 ms is why the device opens once per session.** It would have swallowed the first word of every turn.
- **A frame's place in the audio timeline and a frame's lateness are two different numbers, and one field cannot be both.** The timeline must keep its 20 ms spacing or a turn delivered in one chunk reports a compressed length and `perceived_response_ms`'s zero point moves; the lateness must discount the constant buffering or the flush becomes a standing **−480 ms** on every reading and a real stall can never show as positive. `MicFrame` now carries both, and the delay is the standard one-way delay-variation measure — offset against the smallest offset seen. Verified on the same device: timeline span 3460 ms for 3460 ms of audio, delays min 0 / median 212 / max 498, **zero negative**.

**The demo host's only audio device delivers silence.** `Microphone (Iriun Webcam)` enumerates, opens, and produces frames at peak RMS 0 with its phone application not running. That is the failure `TURN_ONSET_TIMEOUT_MS` and `human_rep_turn_missing` exist for, and it is live on this host right now: a rehearsal on it would look like a slow agent rather than a muted microphone. §6.6's recordings need a working microphone first.
...
| **A-27** | `HOLD_CUE` phrases spoken without a hold do not mute the agent excessively | **Major** | 20 utterances of "let me check" with the persona continuing. `gate_false_close_count` recorded; `agent_mute_during_conversation_ms` p90 < 1500 ms. **Measured 2026-09-26 against perfect transcripts (`pnpm a27`): 20/20 close the gate, 20/20 reopen after exactly two turns, none latches. THE BAR CANNOT BE MET — see §6.3. It is not an implementation failure: the cost is structurally two far-end turns, and the shortest representative line in the catalogue is 1872 ms. The bar and §6.5 were never reconciled, and the owner decides which changes** |
